...
echo "writing amivar-tomcat-prod.tf and uploading it to s3"
echo 'variable "AMI_ID" { default = "'${AMI_ID}'" }' > amivar-tomcat-prod.tf

AWS_REGION="eu-central-1"
S3_BUCKET=`aws s3 ls --region $AWS_REGION | grep terraform-state-tomcat-prod | tail -n1 |cut -d ' ' -f3`
if [ -n "$S3_BUCKET" ]; then
